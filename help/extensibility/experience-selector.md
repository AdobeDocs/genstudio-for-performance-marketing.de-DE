...
>[!TIP]
>
> **API-Service verfügbar** Die GenStudio-API bietet eine RESTful-API für die Verwaltung von Erlebnissen. Wenn Sie die GenStudio-API zum Verwalten von Erlebnissen verwenden möchten, lesen Sie bitte die [GenStudio-API-Dokumentation](https://developer.adobe.com/genstudio-api/).
...
Informationen zum Einbetten dieses MFE in Salesforce Lightning mit LWC und optionalen E-Mail-Vorlagen finden Sie unter [Experience Selector MFE in Salesforce](experience-selector-salesforce.md).
...
Wenn ein [!DNL Experience] im MFE ausgewählt und auf die Schaltfläche **[!UICONTROL Verwenden]** geklickt wird, muss eine Rückruffunktion aufgerufen werden, um die Details des ausgewählten [!DNL Experience] zu übergeben.

Der Callback stellt eine HTML-Zeichenfolge bereit, die den [!DNL Experience] darstellt. Wenn das Erlebnis Assets (z. B. Bilder, Videos) enthält, dann enthalten HTML-Elemente wie `<img>` und `<video>` `src`, die vorsignierte URLs für diese Assets enthalten. Diese vorsignierten URLs haben eine Ablaufzeit von 30 Minuten. Die [!DNL Experience] sollte in dieser Zeit von der Client-Anwendung genutzt werden.

### Ausgabeschema

Der `onSelectionConfirmed`-Callback empfängt ein Array von [!DNL Experience]-Objekten mit der folgenden Struktur:

| Eigenschaft | Typ | Beschreibung |
|----------|------|-------------|
| `id` | `string` | Eindeutige Kennung für die [!DNL Experience] (URN-Format). |
| `content` | `string` | Base64-kodierte HTML-Zeichenfolge, die die [!DNL Experience] darstellt. |
| `experienceFields` | `object` | Schlüssel-Wert-Paare bearbeitbarer Felder in der [!DNL Experience]. Jedes Feld in `experienceFields` enthält: <ul><li>`fieldName`: Der Name/die Kennung des Felds.</li><li>`fieldValue`: Der tatsächliche Inhalt/Wert des Felds.</li></ul> |
| `metadata` | `object` | Metadaten zum Erlebnis (z. B. `channel`, `externalTemplateMetadata`, `externalAssetMetadata` usw.) |
| `aspectVariants` | `array` | Array von Varianten des Seitenverhältnisses für die [!DNL Experience]. Jede Variante enthält: <ul><li>`aspectKey`: Eindeutige Kennung für die Variante.</li><li>`aspectMetadata`: Metadaten, einschließlich `channel` und `aspectRatio`.</li><li>`content`: Base64-kodierte HTML-Zeichenfolge für dieses spezifische Seitenverhältnis.</li></ul> |

### Beispiel-JSON

Nachfolgend finden Sie einen JSON-Beispielblock mit den Schemata.
