---
user-guide-title: Adobe GenStudio for Performance Marketing Benutzerhandbuch
breadcrumb-title: Benutzerhandbuch
user-guide-description: Erfahren Sie mehr über die Funktionen von Adobe GenStudio for Performance Marketing. Erfahren Sie, wie Sie schnell markenkonforme Assets erstellen, Varianten generieren und Erlebnisse optimieren können.
feature-set: GenStudio for Performance Marketing
feature: Generative AI
role: User
nudge: true
source-git-commit: 378b5c689ce9dab9ffdde6d17a06df68cca2d305
workflow-type: tm+mt
source-wordcount: '293'
ht-degree: 95%
---

# Adobe GenStudio for Performance Marketing Guide {#user-guide}

+ [Handbuch zu GenStudio for Performance Marketing](home.md)
+ [Versionshinweise](release-notes.md)
+ [Jetzt loslegen](get-started.md)
+ Einführung {#intro}
  + [Konzepte](concepts.md)
  + [Produktbereitstellung](product-provisioning.md)
  + [Konfigurieren [!DNL Brand] Berechtigungen](configure-brand-permissions.md)
  + [Benutzerrollen](user-roles.md)
  + [Effektive Eingabeaufforderungen](effective-prompts.md)
+ Einstellungen {#settings}
  + [Bezahlte Medien verbinden](connectors/connect-channel.md)
  + Bezahlte Medienkonten {#connect-account}
    + [Google Campaign Manager 360](connectors/google-cm360.md)
    + [LinkedIn](connectors/linkedin-ads.md)
    + [Meta-Anzeigen](connectors/meta-ads.md)
+ Richtlinien {#guidelines}
  + [Richtlinien - Überblick](guidelines/overview.md)
  + [Marken](guidelines/brands.md)
  + [Personas](guidelines/personas.md)
  + [Produkte](guidelines/products.md)
  + [Richtlinien hinzufügen](guidelines/add-guidelines.md)
  + [Markenvalidierung](guidelines/brand-validation.md)
+ anlegen {#create}
  + [[!DNL Create] Überblick](create/overview.md)
  + [E-Mail-Erlebnisse](create/email-experiences.md)
  + [Meta-Erlebnisse](create/meta-experiences.md)
  + [LinkedIn-Erlebnisse](create/linkedin-experiences.md)
  + [TikTok-Erlebnisse](create/tiktok-experiences.md)
  + [Connected TV-Anzeigen](create/create-ctv.md)
  + [Display-Anzeigen-Erlebnisse](create/display-ad-experiences.md)
  + [Banner-Erlebnisse](create/banner-experiences.md)
  + [Varianten verwalten](create/manage-variants.md)
  + [Übersetzen und Lokalisieren von Erlebnissen](create/translate-experiences.md)
  + [Kollaboratives Bearbeiten](create/collaborative-editing.md)
  + [Verwenden des Logo-Tauschs](create/logo-swap.md)
  + [Einrichten des Logo-Tauschs in Vorlagen](create/logo-swap-setup.md)
  + [Erstellen Sie ein E-Mail-Erlebnis](create/create-email-experience.md)
  + [Erstellen eines Meta Ad-Erlebnisses](create/create-meta-ad.md)
  + [Erstellen eines Meta-Anzeigenerlebnisses - Karussellanzeigen](create/create-meta-carousel-ad.md)
  + [Erstellen eines ChatGPT-Anzeigenerlebnisses](create/create-chatgpt-ad.md)
  + [Erstellen Sie ein LinkedIn-Profil](create/create-linkedin.md)
  + [Erstellen einer Display-Anzeige](create/create-display-ad.md)
  + [Erstellen eines Bannererlebnisses](create/create-banner-experience.md)
  + [Grafik erstellen](create/generate-assets.md)
+ Überprüfungen und Genehmigungen {#approve}
  + [Übersicht über Prüfungen und Genehmigungen](approvals/overview.md)
  + [Workfront Proof-Integration](approvals/proof-integration.md)
  + [Überprüfung und Genehmigung anfordern](approvals/request-review.md)
  + [Inhalte prüfen und bearbeiten](approvals/review-and-edit.md)
  + [Inhalte genehmigen](approvals/approve-content.md)
  + [Genehmigte Inhalte veröffentlichen](approvals/publish-content.md)
+ Inhalt {#content}
  + [Inhaltsübersicht](content/overview.md)
  + [Assets und Erlebnisse verwalten](content/manage-assets.md)
  + [Verbinden eines AEM-Repositorys](content/connect-aem-repo.md)
  + [Inhalte-Details](content/asset-details.md)
  + [Content Credentials](content/content-credentials.md)
+ Vorlagen {#templates}
  + [Arbeiten mit Vorlagen](templates/use-templates.md)
  + [Starter-Vorlagen](templates/starter-templates.md)
  + [Vorlage anpassen](templates/customize-template.md)
  + [Vorlagen-Code-Editor](templates/code-editor.md)
  + [Erstellen barrierefreier Vorlagen](templates/accessibility-for-templates.md)
  + [Verwenden von Express-Vorlagen](/help/user-guide/templates/express-templates.md)
  + [Best Practices für Vorlagen](templates/best-practices-for-templates.md)
  + Best Practice für Kanäle {#channel-best-practice}
    + [Anleitung zur Anzeige von Anzeigen](templates/display-template.md)
    + [E-Mail-Handbuch](templates/email-template.md)
    + [LinkedIn-Werbeanzeigen-Handbuch](templates/linkedin-template.md)
    + [Handbuch zu Meta Ads](templates/meta-template.md)
+ Kampagnen {#campaigns}
  + [Kampagnen – Überblick](campaigns/overview.md)
  + [Kampagnen erstellen](campaigns/create-campaign.md)
  + [Kampagnen verwalten](campaigns/manage-campaign.md)
+ Aktivieren {#activation}
  + [Aktivierung – Übersicht](activation/overview.md)
  + [Aktivierungs-Workflow](activation/create-activation.md)
  + [Aktivierungen verwalten](activation/manage-activations.md)
  + [Optimieren von Aktivierungen](activation/troubleshooting.md)
  + Bezahlte Medien-Anzeigen {#paid-media-ads}
    + [Google Campaign Manager 360](activation/activate-cm360-ad.md)
    + [LinkedIn](activation/activate-linkedin-ad.md)
    + [Meta](activation/activate-meta-ad.md)
    + [Amazon](activation/activate-amazon-ad.md)
    + [Innovid](activation/activate-innovid-ad.md)
    + [TikTok](activation/activate-tiktok-ad.md)
    + [YouTube](activation/activate-youtube-ad.md)
    + [ChatGPT](activation/activate-chatgpt-ad.md)
    + [The Trade Desk](activation/activate-trade-desk-ad.md)
+ Insights {#insights}
  + [Insights-Überblick](insights/overview.md)
  + [Insights-Filter](insights/filter-views.md)
  + [Kampagnen](insights/campaigns.md)
  + [Details zu veröffentlichten Erlebnissen](insights/published-experiences.md)
  + [Medien](insights/media.md)
  + [Anzeigen-Tags](insights/ad-tags.md)
  + Attribute {#attributes}
    + [Informationen zu Attributen](insights/attributes.md)
    + [Bildfunktionen](insights/image-features.md)
    + [Videofunktionen](insights/video-features.md)
    + [Textfunktionen](insights/text-features.md)
  + [Daten-Management](insights/data-management.md)
+ Integrationen {#integrations}
  + [Integrationen – Überblick](integrations/overview.md)
  + [GenStudio-Photoshop-Plug-in](integrations/photoshop-plugin.md)
  + [Figma-Plug-in](integrations/figma-plugin.md)
+ {hide-from-toc}[GenStudio Academy – Ausgeblendet](genstudioacademy.md)
+ {hide-from-toc}[Test-Markdown – Ausgeblendet](test-markdown.md)
